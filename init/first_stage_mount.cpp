...
void parse_fstab_file(const std::string &filepath)
{
	...
	if (!file.is_open()) {
		LOGE("Unable to open fstab file: '%s'. Check file path or permissions.",
		     filepath.c_str());
		return;
	}
	...
	while (std::getline(file, line)) {
		...
		if (!(iss >> device >> mount_point >> filesystem >> options)) {
			LOGE("Failed to parse fstab entry on line %d. Skipping.",
			     line_number);
			continue;
		}
		...
		if (filesystem == "overlay") {
			...
			if (!minimal_systems::fs_mgr::MountOverlayFs(
				    mount_point)) {
		...
		if (!minimal_systems::fs_mgr::MountPartition(
			    device, mount_point, filesystem, options)) {
...
bool load_fstab(const std::vector<std::string> &fstab_paths)
{
	...
	for (const auto &path : fstab_paths) {
		...
		if (file) {
			LOGI("fstab '%s' found for hardware '%s' without normalization",
		...
		if (file) {
			LOGI("fstab '%s' found for hardware '%s' with normalization",
			     normalized_path.c_str(), hardware.c_str());
			parse_fstab_file(normalized_path);
			return true;
		}

		LOGW("fstab '%s' not found; continuing to next option",
		     path.c_str());
	}
	...
	if (fallback_file) {
		LOGI("Using fallback fstab '/etc/fstab' for hardware '%s'",
		     hardware.c_str());
...
bool FirstStageMount()
{
	...
	const std::vector<std::string> fstab_paths = { "etc/fstab",
						       "usr/share/etc/fstab",
						       "/etc/fstab" };
