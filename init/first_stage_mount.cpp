    ...
    while (std::getline(file, line)) {
        ...
        if (filesystem == "overlay") {
            ...
            if (!minimal_systems::fs_mgr::MountOverlayFs(mount_point)) {
                LOGE("Overlay mount failed for mount point '%s' on line %d. Aborting.", mount_point.c_str(), line_number);
                std::exit(-1);
            }
        ...
        if (stat(device.c_str(), &buffer) != 0) {
            LOGW("Device '%s' does not exist or cannot be opened. Proceeding with mount attempt.", device.c_str());
        }

        // Attempt to mount the partition
        if (!minimal_systems::fs_mgr::MountPartition(device, mount_point, filesystem, options)) {
            LOGE("Mount operation failed for device '%s' at '%s' on line %d. Aborting.", device.c_str(), mount_point.c_str(), line_number);
            std::exit(-1);
        }
    }
...
bool load_fstab(const std::vector<std::string>& fstab_paths) {
    ...
    for (const auto& path : fstab_paths) {
        ...
        if (file) {
            LOGI("fstab '%s' found for hardware '%s' without normalization", path.c_str(), hardware.c_str());
        ...
        if (file) {
            LOGI("fstab '%s' found for hardware '%s' with normalization", normalized_path.c_str(), hardware.c_str());
