...
bool GetKernelCmdline(const std::string& key, std::string* value) {
  ...
  while (iss >> token) {
    ...
    if (pos != std::string::npos) {
      ...
      if (k == key) {
        *value = v;
        LOGD("Found kernel cmdline key '%s' with value '%s'.", key.c_str(),
             value->c_str());
        return true;
      }
    }
...
bool GetBootconfig(const std::string& key, std::string* value) {
  ...
  while (std::getline(bootconfig_file, line)) {
    ...
    if (pos != std::string::npos) {
      ...
      if (k == key) {
        *value = v;
        LOGD("Found bootconfig key '%s' with value '%s'.", key.c_str(),
             value->c_str());
        return true;
      }
    }
...
bool MountPartition(const std::string& device, const std::string& mount_point,
  ...
  if (filesystem == "ext4" || filesystem == "fat32") {
    ...
    LOGI(
        "[libfs_mgr] Delegating overlay filesystem mounting to MountOverlayFs "
        "for %s.",
        mount_point.c_str());
  ...
  LOGI(
      "[libfs_mgr] Attempting to mount %s at %s with filesystem type %s and "
      "options: %s.",
...
bool MountOverlayFs(const std::string& mount_point) {
  ...
  const std::string overlay_options = "lowerdir=" + lowerdir + ":" +
                                      updater_path + ",upperdir=" + upperdir +
                                      ",workdir=" + workdir;
