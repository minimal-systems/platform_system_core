...
bool MountPartition(const std::string& device, const std::string& mount_point,
    ...
    if (filesystem == "ext4" || filesystem == "fat32") {
        LOGI("[libfs_mgr] Running filesystem check on %s with type %s.", device.c_str(),
             filesystem.c_str());
        if (!FsckPartition(device, filesystem)) {
            LOGE("[libfs_mgr] Filesystem check failed on %s with type %s.", device.c_str(),
                 filesystem.c_str());
            return false;
        }
        LOGI("[libfs_mgr] Filesystem check succeeded on %s with type %s.", device.c_str(),
             filesystem.c_str());
        ...
        LOGW("[libfs_mgr] Unsupported filesystem type %s on %s. Mount skipped.", filesystem.c_str(),
             device.c_str());
        return false;
    }
  ...
  */
    ...
    LOGI("[libfs_mgr] __mount(source=%s,target=%s,type=%s)=0: Success.", device.c_str(),
         mount_point.c_str(), filesystem.c_str());
    LOGI("[libfs_mgr] Mount operation completed for %s at %s.", device.c_str(),
         mount_point.c_str());
    return true;
}
...
bool MountOverlayFs(const std::string& mount_point) {
    ...
    const std::string overlay_options = "lowerdir=" + lowerdir + ":" + updater_path +
