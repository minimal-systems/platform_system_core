...
Result<uid_t> DecodeUid(const std::string &name)
{
	...
	char *endptr = nullptr;
	uid_t result =
		static_cast<uid_t>(std::strtoul(name.c_str(), &endptr, 0));
...
void SetStdioToDevNull(char **argv)
{
	...
	if (dup2(fd_null, STDIN_FILENO) == -1) {
		int saved_errno = errno;
		LOGE("Failed to redirect stdin to /dev/null: %s",
	...
	LOGI("Standard I/O successfully redirected: stdin -> /dev/null, stdout/stderr "
	     "-> /dev/tty");
}
