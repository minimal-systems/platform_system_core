...
const std::vector<std::string> init_dirs = { "etc/init/", "usr/share/etc/init/",
					     "oem/etc/init/" };
...
bool parse_init_files(const std::string &dir_path)
{
	try {
		if (!fs::exists(dir_path)) {
			LOGE("Unable to read config directory '%s': No such file or directory",
			     dir_path.c_str());
			return false;
		}
		...
		LOGE("Exception while parsing directory '%s': %s",
		     dir_path.c_str(), ex.what());
		return false;
	}
}
...
bool file_exists(const std::string &filename)
{
	try {
		...
		LOGE("Exception while checking file '%s': %s", filename.c_str(),
		     ex.what());
		return false;
	}
}
...
bool recovery_init()
{
	...
	const std::string filename =
		props.get("ro.recovery.init_file", "init.rc");

	if (!file_exists(filename)) {
		LOGE("Unable to read recovery init file '%s': No such file or directory",
		     filename.c_str());
		return false;
	}
	...
	try {
		...
		LOGE("Exception while executing recovery init file '%s': %s",
...
bool parse_init()
{
	try {
		...
		if (boot_mode == "recovery") {
			LOGI("Boot mode is 'recovery'; performing recovery-specific "
			     "initialization.");
		...
		for (const auto &dir : init_dirs) {
			if (!parse_init_files(dir)) {
				LOGW("Failed to parse init directory: %s",
				     dir.c_str());
			}
		}
