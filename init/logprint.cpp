...
typedef struct FilterInfo_t {
	char *mTag;
	linux_LogPriority mPri;
	struct FilterInfo_t *p_next;
} FilterInfo;

struct LinuxLogFormat_t {
	linux_LogPriority global_pri;
	FilterInfo *filters;
...
/*
 * API issues prevent us from exposing "descriptive" in LINUXLogFormat_t
 * during LINUX_log_processBinaryLogBuffer(), so we break layering.
 */
...
static FilterInfo *filterinfo_new(const char *tag, linux_LogPriority pri)
{
	FilterInfo *p_ret;
 ...
 * returns LINUX_LOG_UNKNOWN if the character is unrecognized
 */
static linux_LogPriority filterCharToPri(char c)
{
	...
	if (c >= '0' && c <= '9') {
		if (c >= ('0' + LINUX_LOG_SILENT)) {
			pri = LINUX_LOG_VERBOSE;
		} else {
			pri = (linux_LogPriority)(c - '0');
		}
	} else if (c == 'v') {
		pri = LINUX_LOG_VERBOSE;
	} else if (c == 'd') {
		...
		pri = LINUX_LOG_SILENT;
	} else if (c == '*') {
		pri = LINUX_LOG_DEFAULT;
	} else {
		pri = LINUX_LOG_UNKNOWN;
	}
...
static char filterPriToChar(linux_LogPriority pri)
{
    ...
    case LINUX_LOG_WARN:    return 'W';
    case LINUX_LOG_ERROR:   return 'E';
    case LINUX_LOG_FATAL:   return 'F';
    case LINUX_LOG_SILENT:  return 'S';

    case LINUX_LOG_DEFAULT:
    case LINUX_LOG_UNKNOWN:
    default:                  return '?';
		/* clang-format on */
	}
}
...
static int colorFromPri(linux_LogPriority pri)
{
    ...
    case LINUX_LOG_WARN:    return LINUX_COLOR_YELLOW;
    case LINUX_LOG_ERROR:   return LINUX_COLOR_RED;
    case LINUX_LOG_FATAL:   return LINUX_COLOR_RED;
    case LINUX_LOG_SILENT:  return LINUX_COLOR_DEFAULT;

    case LINUX_LOG_DEFAULT:
    case LINUX_LOG_UNKNOWN:
    default:                  return LINUX_COLOR_DEFAULT;
		/* clang-format on */
	}
}
...
{
	LinuxLogFormat *p_ret;

	p_ret = static_cast<LinuxLogFormat *>(
		calloc(1, sizeof(LinuxLogFormat)));

	p_ret->global_pri = LINUX_LOG_VERBOSE;
	p_ret->format = FORMAT_BRIEF;
	p_ret->colored_output = true;
...
void linux_log_format_free(LinuxLogFormat *p_format)
{
	...
	while (!list_empty(&convertHead)) {
		...
		list_remove(node);
		LOG_ALWAYS_FATAL_IF(node == list_head(&convertHead),
				    "corrupted list");
		free(node);
	}
}
...
LinuxLogPrintFormat linux_log_formatFromString(const char *formatString)
{
	/* clang-format off */
			      ...
			      const char *filterString)
{
	...
	while (NULL != (p_ret = strsep(&p_cur, " \t,"))) {
		...
		if (p_ret[0] != '\0') {
			err = LINUX_LOG_addFilterRule(p_format, p_ret);
...
int linux_log_processLogBuffer(struct logger_entry *buf, LinuxLogEntry *entry)
{
   ...
   */
	...
	if (msgStart == -1) {
		...
		if (msgStart == -1) {
			msgStart = buf->len -
				   1; /* All tag, no message, print truncates */
		}
	}
	if (msgEnd == -1) {
	...
	entry->priority = static_cast<linux_LogPriority>(msg[0]);
	entry->tag = msg + 1;
				      ...
				      size_t *fmtLen)
{
   ...
   */
	if (!cp || !findChar(&cp, &len, '(')) {
		...
		if (findChar(&cp, &len, '|') && findChar(&cp, &len, INT_MAX)) {
			static const unsigned char typeTable[] = {
				EVENT_TYPE_INT, EVENT_TYPE_LONG,
				EVENT_TYPE_STRING, EVENT_TYPE_LIST,
				EVENT_TYPE_FLOAT
			};

			if ((*cp >= '1') &&
			    (*cp < (char)('1' + (sizeof(typeTable) /
						 sizeof(typeTable[0])))) &&
	...
	case EVENT_TYPE_INT:
		/* 32-bit signed int */
		{
			if (eventDataLen < sizeof(linux_event_int_t))
				return -1;
			auto *event_int =
				reinterpret_cast<const linux_event_int_t *>(
					eventData);
			lval = event_int->data;
	...
	case EVENT_TYPE_LONG:
		/* 64-bit signed long */
		if (eventDataLen < sizeof(linux_event_long_t)) {
			return -1;
		}
		{
			auto *event_long =
				reinterpret_cast<const linux_event_long_t *>(
					eventData);
			lval = event_long->data;
		}
		eventData += sizeof(linux_event_long_t);
		eventDataLen -= sizeof(linux_event_long_t);
	pr_lval:
	...
	case EVENT_TYPE_FLOAT:
		/* float */
		{
			if (eventDataLen < sizeof(linux_event_float_t))
				return -1;
			auto *event_float =
				reinterpret_cast<const linux_event_float_t *>(
					eventData);
	...
	case EVENT_TYPE_STRING:
		...
		{
			if (eventDataLen < sizeof(linux_event_string_t))
				return -1;
			auto *event_string =
				reinterpret_cast<const linux_event_string_t *>(
					eventData);
			...
			if (strLen < outBufLen) {
				...
				if (!result)
					result =
						1; /* if not truncated, return no room */
			}
			eventData += strLen;
			eventDataLen -= strLen;
			if (result != 0)
	...
	case EVENT_TYPE_LIST:
		...
		{
			if (eventDataLen < sizeof(linux_event_list_t))
				return -1;
			auto *event_list =
				reinterpret_cast<const linux_event_list_t *>(
					eventData);

			int8_t count = event_list->element_count;
			eventData += sizeof(linux_event_list_t);
			eventDataLen -= sizeof(linux_event_list_t);
 ...
 * here.
 */
int linux_log_processBinaryLogBuffer(
	struct logger_entry *buf, LinuxLogEntry *entry,
	...
	char *messageBuf, int messageBufLen)
{
	...
	if (buf->lid == LOG_ID_SECURITY) {
		entry->priority = LINUX_LOG_WARN;
	}
	entry->uid = buf->uid;
	inCount = buf->len;
	if (inCount < sizeof(linux_event_header_t))
		return -1;
	auto *event_header =
		reinterpret_cast<const linux_event_header_t *>(eventData);
	tagIndex = event_header->tag;
	eventData += sizeof(linux_event_header_t);
	inCount -= sizeof(linux_event_header_t);
   ...
   */
	if (entry->tag == NULL) {
		...
		tagLen = snprintf(messageBuf, messageBufLen, "[%" PRIu32 "]",
				  tagIndex);
   ...
   */
	...
	if ((inCount > 0) || fmtLen) {
		result = linux_log_printBinaryEvent(&eventData, &inCount,
						    &outBuf, &outRemaining,
						    &fmtStr, &fmtLen);
	}
	if ((result == 1) && fmtStr) {
		...
		eventData =
			reinterpret_cast<unsigned char *>(buf) + buf->hdr_size;
		eventData += 4;
		outBuf = messageBuf;
...
size_t convertPrintable(char *dst0, const char *src0, size_t n)
{
	...
	const unsigned char *src =
		reinterpret_cast<const unsigned char *>(src0);
	mbstate_t mbs = {};
	...
	while (n > 0) {
		...
		ssize_t len = mbrtowc(
			nullptr, reinterpret_cast<const char *>(src), n, &mbs);
		if (len > 0) {
			if (print)
				memcpy(dst, src, len);
			dst += len;
			     ...
			     const AndroidLogEntry *entry)
{
     ...
     */
		...
		if (p) {
			...
			while (getline(&line, &len, p) > 0) {
				...
				if (add_entry) {
					...
					list = static_cast<conversionList *>(
						calloc(1,
						       sizeof(conversionList)));
					list_init(&list->node);
					list->time = time;
					subTimespec(&list->convert, &time,
						    &monotonic);
					list_add_tail(&convertHead,
						      &list->node);
				}
				if (suspended_pending &&
				    !list_empty(&convertHead)) {
					list = node_to_item(
						list_tail(&convertHead),
					...
					list_init(&list->node);
					list->time = time;
					sumTimespec(&list->time,
						    &suspended_diff);
					list->convert = convert;
					sumTimespec(&list->convert,
						    &suspended_diff);
					list_add_tail(&convertHead,
						      &list->node);
		...
		list = static_cast<conversionList *>(
			calloc(1, sizeof(conversionList)));
		list_init(&list->node);
		...
		subTimespec(&time, &convert,
			    subTimespec(&time, &time, &convert));
	...
	list = node_to_item(list_head(&convertHead), struct conversionList,
			    node);
	next = NULL;
	...
	if (next) {
		...
		total = nsecTimespec(
			subTimespec(&time, &next->time, &list->time));
		...
		if (run < total) {
			...
			float f = nsecTimespec(
				subTimespec(&time, &next->convert, &convert));
			f *= run;
			f /= total;
			crun = f;
			      ...
			      const LinuxLogEntry *entry, size_t *p_outLength)
{
   ...
   */
	...
	if (p_format->nsec_time_output) {
		len += snprintf(timeBuf + len, sizeof(timeBuf) - len, ".%09ld",
				nsec);
   ...
   */
	...
	case FORMAT_TAG:
		len = snprintf(prefixBuf + prefixLen,
			       sizeof(prefixBuf) - prefixLen,
			       "%c/%-8.*s: ", priChar, (int)entry->tagLen,
			       entry->tag);
		strcpy(suffixBuf + suffixLen, "\n");
		++suffixLen;
		break;
	case FORMAT_PROCESS:
		len = snprintf(suffixBuf + suffixLen,
			       sizeof(suffixBuf) - suffixLen, "  (%.*s)\n",
			       (int)entry->tagLen, entry->tag);
		suffixLen += std::min(len, sizeof(suffixBuf) - suffixLen);
		len = snprintf(prefixBuf + prefixLen,
			       sizeof(prefixBuf) - prefixLen, "%c(%s%5d) ",
			       priChar, uid, entry->pid);
		break;
	case FORMAT_THREAD:
		len = snprintf(prefixBuf + prefixLen,
			       sizeof(prefixBuf) - prefixLen, "%c(%s%5d:%5d) ",
	...
	case FORMAT_THREADTIME:
		...
		len = snprintf(prefixBuf + prefixLen,
			       sizeof(prefixBuf) - prefixLen,
			       "%s %s%5d %5d %c %-8.*s: ", timeBuf, uid,
			       entry->pid, entry->tid, priChar,
			       (int)entry->tagLen, entry->tag);
		strcpy(suffixBuf + suffixLen, "\n");
		++suffixLen;
		break;
   ...
   */
	...
	if (p_format->printable_output) {
		...
		bufferSize += convertPrintable(NULL, entry->message,
					       entry->messageLen);
	} else {
	...
	if (prefixSuffixIsHeaderFooter) {
		...
		p += prefixLen;
		if (p_format->printable_output) {
			p += convertPrintable(p, entry->message,
					      entry->messageLen);
		} else {
			      ...
			      const LinuxLogEntry *entry)
{
	...
	size_t line_length;
	char *line = linux_log_formatLogLine(p_format, buf, sizeof(buf), entry,
					     &line_length);
	if (!line) {
