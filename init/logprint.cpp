...
LinuxLogFormat* linux_log_format_new() {
    ...
    p_ret = static_cast<LinuxLogFormat*>(calloc(1, sizeof(LinuxLogFormat)));
...
static int linux_log_printBinaryEvent(const unsigned char** pEventData, size_t* pEventDataLen,
    ...
    if (!cp || !findChar(&cp, &len, '(')) {
        ...
        if (findChar(&cp, &len, '|') && findChar(&cp, &len, INT_MAX)) {
            static const unsigned char typeTable[] = {EVENT_TYPE_INT, EVENT_TYPE_LONG,
    ...
    switch (type) {
        case EVENT_TYPE_INT:
            /* 32-bit signed int */
            {
                if (eventDataLen < sizeof(linux_event_int_t)) return -1;
                auto* event_int = reinterpret_cast<const linux_event_int_t*>(eventData);
                lval = event_int->data;
            ...
            /* N items, all different types */
            {
                ...
                *outBuf++ = '[';
                ...
                for (int i = 0; i < count; i++) {
                    result = linux_log_printBinaryEvent(&eventData, &eventDataLen, &outBuf,
                ...
                case TYPE_MONOTONIC: {
                    ...
                    outCount =
                            snprintf(outBuf, outBufLen,
                                     (val >= minute) ? "%02" PRIu64 : "%" PRIu64 "s", val % minute);
                } break;
...
int linux_log_processBinaryLogBuffer(
    ...
    entry->uid = buf->uid;
    inCount = buf->len;
    if (inCount < sizeof(linux_event_header_t)) return -1;
    auto* event_header = reinterpret_cast<const linux_event_header_t*>(eventData);
    ...
    if ((result == 1) && fmtStr) {
        ...
        result = linux_log_printBinaryEvent(&eventData, &inCount, &outBuf, &outRemaining, NULL,
                                            NULL);
    }
    if (result < 0) {
...
char* linux_log_formatLogLine(LinuxLogFormat* p_format, char* defaultBuffer,
    ...
    if (p_format->epoch_output || p_format->monotonic_output) {
        ...
        strftime(timeBuf, sizeof(timeBuf), &"%Y-%m-%d %H:%M:%S"[p_format->year_output ? 0 : 3],
                 ptm);
    }
    len = strlen(timeBuf);
    ...
    switch (p_format->format) {
        ...
        case FORMAT_PROCESS:
            ...
            len = snprintf(prefixBuf + prefixLen, sizeof(prefixBuf) - prefixLen, "%c(%s%5d) ",
        ...
        case FORMAT_TIME:
            len = snprintf(prefixBuf + prefixLen, sizeof(prefixBuf) - prefixLen,
                           "%s %c/%-8.*s(%s%5d): ", timeBuf, priChar, (int)entry->tagLen,
        ...
        case FORMAT_THREADTIME:
            ...
            len = snprintf(prefixBuf + prefixLen, sizeof(prefixBuf) - prefixLen,
                           "%s %s%5d %5d %c %-8.*s: ", timeBuf, uid, entry->pid, entry->tid,
                           priChar, (int)entry->tagLen, entry->tag);
            strcpy(suffixBuf + suffixLen, "\n");
            ++suffixLen;
            break;
        case FORMAT_LONG:
            len = snprintf(prefixBuf + prefixLen, sizeof(prefixBuf) - prefixLen,
                           "[ %s %s%5d:%5d %c/%-8.*s ]\n", timeBuf, uid, entry->pid, entry->tid,
                           priChar, (int)entry->tagLen, entry->tag);
            strcpy(suffixBuf + suffixLen, "\n\n");
            suffixLen += 2;
        ...
        default:
            len = snprintf(prefixBuf + prefixLen, sizeof(prefixBuf) - prefixLen,
                           "%c/%-8.*s(%s%5d): ", priChar, (int)entry->tagLen, entry->tag, uid,
                           entry->pid);
            strcpy(suffixBuf + suffixLen, "\n");
            ++suffixLen;
            break;
