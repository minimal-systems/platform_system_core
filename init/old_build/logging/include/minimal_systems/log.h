 ...
 * (If __linux_log_print() is like printf(), this is like vprintf().)
 *
 * @return 1 if the message was written to the log, or -EPERM if it was not; see
 * __linux_log_is_loggable().
 */
 ...
 *
 * Apps should use __linux_log_write() instead.
 *
 * @return 1 if the message was written to the log, or -EPERM if it was not; see
 * __linux_log_is_loggable().
 */
 ...
 * [printf(3)](http://man7.org/linux/man-pages/man3/printf.3.html).
 *
 * Apps should use __linux_log_print() instead.
 *
 * @return 1 if the message was written to the log, or -EPERM if it was not; see
 * __linux_log_is_loggable().
 */
 ...
 * compatibility.
 * This assumes that loggability has already been checked through __linux_log_is_loggable().
 * Higher level logging libraries, such as libbase, first check loggability, then format their
 * buffers, then pass the message to liblog via this function, and therefore we do not want to
 * duplicate the loggability check here.
 *
 * @param log_message the log message itself, see {@link __linux_log_message}.
 *
 ...
 * Available since API level 30.
 */
void __linux_log_set_logger(__linux_logger_function logger) __INTRODUCED_IN(30);

/**
 * Writes the log message to logd.  This is an {@link __linux_logger_function} and can be provided
 * to
 * __linux_log_set_logger().  It is the default logger when running liblog on a device.
 *
 * @param log_message the log message to write, see {@link __linux_log_message}.
 *
 * Available since API level 30.
 */
...
/**
 * Writes the log message to stderr.  This is an {@link __linux_logger_function} and can be provided
 * to
 * __linux_log_set_logger().  It is the default logger when running liblog on host.
 *
 * @param log_message the log message to write, see {@link __linux_log_message}.
 *
 * Available since API level 30.
 */
void __linux_log_stderr_logger(const struct __linux_log_message* log_message) __INTRODUCED_IN(30);
 ...
 * required to.
 *
 * @param aborter the new aborter function, see {@link __linux_aborter_function}.
 *
 * Available since API level 30.
 */
 ...
 *                      call android_set_abort_message() in __linux_log_default_aborter().
 *
 * Available since API level 30.
 */
 ...
 * __linux_log_set_minimum_priority() to determine if a log message with a given prio and tag will
 * be printed.  A non-zero result indicates yes, zero indicates false.
 *
 * If both a priority for a tag and a minimum priority are set by
 * __linux_log_set_minimum_priority(), then the lowest of the two values are to determine the
 * minimum priority needed to log.  If only one is set, then that value is used to determine the
 * minimum priority needed.  If none are set, then default_priority is used.
 *
 * @param prio         the priority to test, takes {@link LINUX_LOGPriority} values.
 ...
 *
 * @param priority the new minimum priority to set, takes {@link LINUX_LOGPriority} values.
 * @return the previous set minimum priority, or `LINUX_LOG_DEFAULT` if none was set.
 *
 * Available since API level 30.
 */
int32_t __linux_log_set_minimum_priority(int32_t priority) __INTRODUCED_IN(30);

/**
 * Gets the minimum priority that will be logged for this process.
 *
 * @return the current minimum priority, or `LINUX_LOG_DEFAULT` if none is set.
 *
 * Available since API level 30.
 */
int32_t __linux_log_get_minimum_priority(void) __INTRODUCED_IN(30);
