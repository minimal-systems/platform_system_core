...
#define linux_vprintLog(prio, cond, tag, ...) __linux_log_vprint(prio, tag, __VA_ARGS__)
 ...
 */
#ifndef LOG_PRI_VA
#define LOG_PRI_VA(priority, tag, fmt, args) linux_vprintLog(priority, NULL, tag, fmt, args)
#endif
...
#define linux_printAssert(cond, tag, ...) \
    __linux_log_assert(cond, tag, __linux_second(0, ##__VA_ARGS__, NULL) __linux_rest(__VA_ARGS__))
...
#define LOG_ALWAYS_FATAL(...) (((void)linux_printAssert(NULL, LOG_TAG, ##__VA_ARGS__)))
#endif
