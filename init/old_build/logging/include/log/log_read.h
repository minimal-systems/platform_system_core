...
#define LINUX_LOG_WRAP_DEFAULT_TIMEOUT 7200 /* 2 hour default */

/*
 * Native log reading interface section.
 */
...
ssize_t linux_logger_get_statistics(struct logger_list* logger_list, char* buf, size_t len);
ssize_t linux_logger_get_prune_list(struct logger_list* logger_list, char* buf, size_t len);
...
struct logger_list* linux_logger_list_alloc(int mode, unsigned int tail, pid_t pid);
struct logger_list* linux_logger_list_alloc_time(int mode, log_time start, pid_t pid);
void linux_logger_list_free(struct logger_list* logger_list);
/* In the purest sense, the following two are orthogonal interfaces */
int linux_logger_list_read(struct logger_list* logger_list, struct log_msg* log_msg);

/* Multiple log_id_t opens */
struct logger* linux_logger_open(struct logger_list* logger_list, log_id_t id);
/* Single log_id_t open */
struct logger_list* linux_logger_list_open(log_id_t id, int mode, unsigned int tail, pid_t pid);
