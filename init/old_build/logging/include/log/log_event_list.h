...
int linux_log_write_string8_len(linux_log_context ctx, const char* value, size_t maxlen);
...
class linux_log_event_list {
  ...
  public:
    ...
    ~linux_log_event_list() { linux_log_destroy(&ctx); }
    ...
    operator linux_log_context() const { return ctx; }
    ...
    linux_log_event_list& operator<<(uint32_t value) {
        int retval = linux_log_write_int32(ctx, static_cast<int32_t>(value));
        if (retval < 0) ret = retval;
        return *this;
    }
    ...
    linux_log_event_list& operator<<(uint64_t value) {
        int retval = linux_log_write_int64(ctx, static_cast<int64_t>(value));
        if (retval < 0) ret = retval;
        return *this;
    }
    ...
    linux_log_event_list& operator<<(const std::string& value) {
        int retval = linux_log_write_string8_len(ctx, value.data(), value.length());
        if (retval < 0) ret = retval;
        return *this;
    }
