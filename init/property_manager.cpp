...
void PropertyManager::loadProperties(const std::string &propertyFile)
{
	...
	if (!propertyFile.empty()) {
		...
		if (!file) {
			DEBUG_LOGE("Failed to open property file: %s",
				   propertyFile.c_str());
			return;
		}
		...
		while (std::getline(file, line)) {
			...
			if (std::getline(lineStream, key, '=') &&
				...
				DEBUG_LOGD(
					"Loaded property into memory: %s = %s",
...
void PropertyManager::saveProperties(const std::string &propertyFile) const
{
	...
	DEBUG_LOGI("Properties saved to file successfully: %s",
		   propertyFile.c_str());
}
...
{
	set(key, value);
	DEBUG_LOGD("Property set via setprop: %s = %s", key.c_str(),
		   value.c_str());
}
...
{
	auto value = get(key, "");
	DEBUG_LOGD("Property get via getprop: %s = %s", key.c_str(),
		   value.c_str());
	return value;
}
