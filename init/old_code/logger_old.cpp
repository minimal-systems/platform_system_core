...
int prepare_log()
{
	...
	if (!logfile.is_open()) {
		std::cerr << ERROR_COLOR << "Couldn't create a logging file"
			  << LOG_END;
		return -2;
	}
	return 0;
...
static int logger(const char *tag, char type, const char *format, va_list args)
{
	...
	if (!tag || !format) {
		std::cerr << ERROR_COLOR << "Null pointer passed to logger"
			  << LOG_END;
		return -1;
	}
...
void SetLogger(
	std::function<int(const char *, char, const char *, va_list)> &&logger)
{
	static std::function<int(const char *, char, const char *, va_list)>
		gLogger;
	...
	for (const auto &spec : specs) {
		if (spec.size() == 3 && spec[1] == ':') {
			...
			default:
				pr_fatal(
					LOG_TAG,
					"Unsupported log level in LINUX_LOG_TAGS: %c",
					level);
			}
		}
	}
...
static void KernelLogLine(LogSeverity severity, const char *tag,
			  const std::string &line)
{
	...
	case FATAL:
		...
		break;
	default:
		pr_err(tag, "Unexpected log severity: %d, message: %s",
		       severity, line.c_str());
		break;
	}
}
...
template <typename F>
void SplitByLines(const char *msg, const F &log_function, LogSeverity severity,
		  const char *tag)
{
...
void KernelLogger(int log_id, LogSeverity severity, const char *tag,
