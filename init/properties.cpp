   ...
   */
	...
	if (tag_len != 0) {
		...
		if (locked) {
			...
			if (last_tag->compare(0, last_tag->size(), tag,
					      tag_len) != 0) {
				// Invalidate log.tag.<tag> cache.
				for (size_t i = 0; i < arraysize(tag_cache);
				     ++i) {
	...
	default:
		...
		for (size_t i = 0; i < arraysize(global_cache); ++i) {
			...
			if (!locked) {
				temp_cache = *cache;
				if (temp_cache.cache.pinfo !=
				    cache->cache.pinfo) { // check atomic
					temp_cache.cache.pinfo = NULL;
					temp_cache.c = '\0';
				}
				cache = &temp_cache;
				...
				int default_prio)
{
	...
	if (property_log_level >= 0 &&
	    minimum_log_priority != ANDROID_LOG_DEFAULT) {
		return prio >=
...
int __linux_log_security()
{
	...
	static cache_char security_prop = { { NULL, 0xFFFFFFFF },
					    BOOLEAN_FALSE };
